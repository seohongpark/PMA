...
def experiment(
    ...
    seed = variant['seed']
    setup_logger(run_group, exp_prefix, variant=variant, seed=seed, log_to_wandb=log_to_wandb)
    ...
    if 'cp_info' in variant['env_kwargs']:
        eval_env_kwargs = copy.deepcopy(variant['env_kwargs'])
        eval_env_kwargs['cp_info']['use_true_env'] = True
        eval_env, _ = make_env(variant['env_name'], **eval_env_kwargs)
    else:
    ...
    sample_latent_every = variant['sample_latent_every']
    if collector_type == 'step':
        ...
        expl_path_collector = LatentPathCollector(
            sample_latent_every=sample_latent_every,
            env=expl_env,
            policy=exploration_policy,
            z_eq_a=variant['z_eq_a'],
            last_ground=variant['last_ground'],
        )
    ...
    if collector_type == 'gcr':
        ...
        eval_path_collector = LatentPathCollector(
            sample_latent_every=sample_latent_every,
            env=eval_env,
        ...
        raise NotImplementedError('collector_type of experiment not recognized')
        eval_path_collector = MdpPathCollector(eval_env, config['evaluation_policy'], deterministic=True)
