...
class BaseRLAlgorithm(object, metaclass=abc.ABCMeta):
    ...
    def _end_epoch(self, epoch):
        ...
        if self.save_snapshot_freq is not None and epoch != 0 and epoch % self.save_snapshot_freq == 0:
            logger.save_itr_params(epoch, snapshot)
        gt.stamp('saving', unique=False)

        if epoch % self.num_epochs_per_log == 0:
    ...
    def _get_training_diagnostics_dict(self):
        diagnostics = self._get_trainer_diagnostics()
        if isinstance(diagnostics, dict):
            return diagnostics
        else:
            return {'policy_trainer': diagnostics}
