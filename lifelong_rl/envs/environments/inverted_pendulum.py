    ...
    def step(self, a, task='default', render=False):
        ...
        if task in ['default', 'forward']:
            if self._terminate_when_unhealthy:
                reward = 1.0 + (ob[0] - ob_before[0]) / self.dt
                done = not notdone
            else:
                reward = notdone + (ob[0] - ob_before[0]) / self.dt / 10
                done = False
        elif task == 'stay':
            if self._terminate_when_unhealthy:
                reward = 1.0 - ob[0] ** 2
    ...
    def compute_reward(self, ob, next_ob, action=None, task='default'):
        ...
        if task in ['default', 'forward']:
            ...
            if self._terminate_when_unhealthy:
                ...
                reward = forward_reward / 10 + notdone
