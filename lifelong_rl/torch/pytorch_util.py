...
def variance_scaling_init(tensor, scale=1):
    size = tensor.size()
    if len(size) == 2:
        fan_in = size[0]
        fan_out = size[1]
...
def variance_scaling_range(fan_in, scale=1):
    return np.sqrt(3. * scale / fan_in)
...
def full(*sizes, fill_value, torch_device=None, **kwargs):
    if torch_device is None:
        torch_device = device
    return torch.full(sizes, fill_value, **kwargs, device=torch_device)
