...
class TanhGaussianPolicy(Mlp, ExplorationPolicy):
    ...
    def __init__(
        ...
        init_w = ptu.variance_scaling_range(hidden_sizes[-1], 0.1)
        super().__init__(
            hidden_sizes,
            input_size=obs_dim - omit_obs_dim,
            output_size=action_dim,
            init_w=init_w,
            hidden_init=ptu.glorot_uniform_unit,
            **kwargs
        )
        self.log_std = None
        self.std = std
        self.omit_obs_dim = omit_obs_dim

        if std is None:
            last_hidden_size = obs_dim - omit_obs_dim
            if len(hidden_sizes) > 0:
                last_hidden_size = hidden_sizes[-1]
            self.last_fc_log_std = nn.Linear(last_hidden_size, action_dim)
            self.last_fc_log_std.weight.data.uniform_(-init_w, init_w)
            self.last_fc_log_std.bias.data.zero_()
        else:
    ...
    def get_log_prob(self, obs, latents, actions):
        # Only used for computing action_mi. Designed to be compatible with skill dynamic's get_log_prob()
        ob_latents = torch.cat([obs, latents], dim=-1)
        _, mean, log_std, *_ = self.forward(ob_latents, deterministic=True)
        tanh_normal = TanhNormal(mean, log_std.exp())
        return tanh_normal.log_prob(actions).sum(dim=-1, keepdim=False)
