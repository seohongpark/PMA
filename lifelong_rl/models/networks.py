...
class OmitMlp(FlattenMlp):
    def __init__(self, input_size, omit_dim, *args, **kwargs):
        super().__init__(
            input_size=input_size - omit_dim,
            *args,
            **kwargs
        )
        self.omit_dim = omit_dim

    def forward(self, *inputs, **kwargs):
        assert len(inputs) == 2  # state, action
        assert len(inputs[0].shape) == 2
        inputs = inputs[0][:, self.omit_dim:], inputs[1]
